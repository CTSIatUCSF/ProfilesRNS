/****** Object:  StoredProcedure [Ontology.].[UpdateCounts]    Script Date: 5/8/2017 3:35:55 PM ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [Ontology.].[UpdateCounts]
AS
BEGIN
	...
	SELECT g.ClassGroupURI, COUNT(DISTINCT t.Subject) NumberOfNodes
		INTO #ClassGroup
		FROM [Ontology.].[ClassGroup] g WITH (NOLOCK), [Ontology.].[ClassGroupClass] c WITH (NOLOCK), [RDF.].[Triple] t WITH (NOLOCK),
			[RDF.].[Node] s WITH (NOLOCK), [RDF.].[Node] p WITH (NOLOCK), [RDF.].[Node] o WITH (NOLOCK)
	...
	SELECT c.ClassGroupURI, c.ClassURI, COUNT(DISTINCT t.Subject) NumberOfNodes
		...
		FROM [Ontology.].[ClassGroupClass] c WITH (NOLOCK), [RDF.].[Triple] t WITH (NOLOCK),
			[RDF.].[Node] s WITH (NOLOCK), [RDF.].[Node] p WITH (NOLOCK), [RDF.].[Node] o WITH (NOLOCK)
	...
	SELECT c.ClassPropertyID, COUNT(DISTINCT t.Subject) NumberOfNodes, COUNT(DISTINCT t2.TripleID) NumberOfTriples
		INTO #ClassProperty
		FROM [Ontology.].[ClassProperty] c WITH (NOLOCK), [RDF.].[Triple] t WITH (NOLOCK),
			[RDF.].[Node] s WITH (NOLOCK), [RDF.].[Node] p WITH (NOLOCK), [RDF.].[Node] o WITH (NOLOCK),
			[RDF.].[Triple] t2 WITH (NOLOCK),
			[RDF.].[Node] p2, [RDF.].[Node] o2 WITH (NOLOCK)
...
BEGIN TRY 
	begin transaction
		...
		UPDATE o
			...
			FROM [Ontology.].[ClassGroup] o WITH (NOLOCK)
		...
		UPDATE o
			...
			FROM [Ontology.].[ClassGroupClass] o WITH (NOLOCK)
		...
		UPDATE o
			...
			FROM [Ontology.].[ClassProperty] o WITH (NOLOCK)
