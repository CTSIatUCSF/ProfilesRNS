/****** Object:  StoredProcedure [RDF.Stage].[ProcessDataMap]    Script Date: 9/21/2017 10:53:33 AM ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [RDF.Stage].[ProcessDataMap]
	@DataMapID INT = NULL,
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			END TRY
			BEGIN CATCH
				declare @error int, @message varchar(4000), @info varchar(4000);
				select @error = ERROR_NUMBER(), @message = ERROR_MESSAGE();
				select @info = '[RDF.Stage].[ProcessDataMap] DataMapID=' + cast(@DataMapID as varchar) + ', : %d: %s';
				raiserror (@info, 16, 1, @error, @message) ;
				return;
			END CATCH
