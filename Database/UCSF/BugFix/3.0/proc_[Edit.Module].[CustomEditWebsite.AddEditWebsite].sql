...
/****** Object:  StoredProcedure [Edit.Module].[CustomEditWebsite.AddEditWebsite]    Script Date: 8/5/2021 8:31:14 AM ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [Edit.Module].[CustomEditWebsite.AddEditWebsite]
...
BEGIN
	...
	BEGIN 
		...
		BEGIN
			-- UCSF, we add new ones with sort order = 1 so that they show up at the top
			UPDATE [Profile.Data].[Person.Websites] SET SortOrder = SortOrder + 1 WHERE PersonID = @InternalID
			INSERT INTO [Profile.Data].[Person.Websites] (UrlID, PersonID, URL, WebPageTitle, SortOrder) 
			VALUES(@ExistingURLID, @InternalID, @URL, @WebPageTitle, 1)
		...
		BEGIN
			-- UCSF, we add new ones with sort order = 1 so that they show up at the top
			UPDATE [Profile.Data].[Person.MediaLinks] SET SortOrder = SortOrder + 1 WHERE PersonID = @InternalID
			INSERT INTO [Profile.Data].[Person.MediaLinks] (UrlID, PersonID, URL, WebPageTitle, PublicationDate, SortOrder) 
			VALUES(@ExistingURLID, @InternalID, @URL, @WebPageTitle, @PublicationDate, 1)
		...
		BEGIN
			-- UCSF, we add new ones with sort order = 1 so that they show up at the top
			UPDATE [Profile.Data].[Group.Websites] SET SortOrder = SortOrder + 1 WHERE GroupID = @InternalID
		...
		BEGIN
			-- UCSF, we add new ones with sort order = 1 so that they show up at the top
			UPDATE [Profile.Data].[Group.MediaLinks] SET SortOrder = SortOrder + 1 WHERE GroupID = @InternalID
