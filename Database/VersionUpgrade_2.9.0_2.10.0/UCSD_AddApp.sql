...
/****** Object:  StoredProcedure [ORNG.].[AddAppToPerson]    Script Date: 4/4/2017 1:30:21 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	BEGIN TRAN
		...
		IF (@PERSON_FILTER_ID IS NOT NULL) 
			BEGIN
				select * from [Profile.Import].[PersonFilterFlag]
				where internalusername in 
					(select internalusername from [Profile.Data].[Person]
						where personid=@PersonID
					)
					and personfilter in
					(select personfilter from [Profile.Data].[Person.Filter]
						where personfilterID=@PERSON_FILTER_ID
					)
				if @@ROWCOUNT=0
					BEGIN
						INSERT [Profile.Import].[PersonFilterFlag]
