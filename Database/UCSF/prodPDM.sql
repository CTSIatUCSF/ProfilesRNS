...
/****** Object:  StoredProcedure [RDF.Stage].[ProcessDataMap]    Script Date: 6/1/2017 12:31:40 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			select x.*, HASHBYTES('sha1',N'"'+CAST(replace(x.Class+N'^^'+x.InternalType+N'^^'+x.InternalID,N'"',N'\"') AS NVARCHAR(4000))+N'"') InternalHash
				FROM #Node x
					WHERE NOT EXISTS (
						SELECT *
						FROM [RDF.Stage].InternalNodeMap m
