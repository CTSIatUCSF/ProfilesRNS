...
ALTER PROCEDURE [Profile.Import].[PRNSWebservice.PubMed.GetPersonInfoForDisambiguation] 
...
SELECT -1, personid, 
                   (SELECT 
										  CASE
											when p.PublishingFirst != p.firstName
												then p.PublishingFirst
											else
												ISNULL(RTRIM(firstname),'')  
										  END	"Name/First",
                                          ISNULL(RTRIM(middlename),'') "Name/Middle",
										  CASE
											when p.PublishingLast != p.LastName
												then p.PublishingLast
											else
												ISNULL(RTRIM(p.Lastname),'')  
										  END	"Name/Last",				   
                                          ...
                                          (SELECT PMID
											...
											and pmid is not NULL and pmid>0											
