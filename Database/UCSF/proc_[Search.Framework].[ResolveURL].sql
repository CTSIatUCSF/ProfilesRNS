/****** Object:  StoredProcedure [Search.Framework].[ResolveURL]    Script Date: 10/12/2022 2:57:03 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	BEGIN
		...
		if(@Param1='advanced' and @Param2='')
		BEGIN
			SELECT @Resolved = 1,
				@ErrorDescription = '',
				@ResponseURL = @REDIRECTPAGE  + '?tab=advanced'				
					
		END		
		if(@Param1='advanced' and @Param2='results')
		BEGIN
			SELECT @Resolved = 1,
				@ErrorDescription = '',
				@ResponseURL = @REDIRECTPAGE  + '?tab=advanced&action=results'				
