...
namespace Profiles.Profile.Modules.SimilarPeopleSameInstitution
{
    public partial class SimilarPeopleSameInstitution : BaseModule
    {
        ...
        public SimilarPeopleSameInstitution() { }
        public SimilarPeopleSameInstitution(XmlDocument pagedata, List<ModuleParams> moduleparams, XmlNamespaceManager pagenamespaces)
        ...
        public void DrawProfilesModule()
        {
            ...
            using (SqlDataReader reader = data.GetSimilarPeopleSameInstitution(Convert.ToInt64(Request.QueryString["subject"])))
            {
                while (reader.Read())
                {
                    // drawstuff
                    ...
                    if (itemurl != string.Empty)
                    {
                        ...
                        documentdata.Append(UCSFIDSet.ByPersonId[Int32.Parse(reader["SimilarPersonID"].ToString())].PrettyURL);
                        documentdata.Append("\"");

                        if (cloudweightnode != string.Empty)
                        {
                            documentdata.Append(" Weight=\"");
                            documentdata.Append(float.Parse(reader["Weight"].ToString()));
                            documentdata.Append("\"");
                        }

                        if ("True".Equals(reader["CoAuthor"].ToString()))
                        {
                            documentdata.Append(" CoAuthor=\"");
                            documentdata.Append(reader["CoAuthor"].ToString().ToLower());
                            documentdata.Append("\"");
                        }

                        string name = reader["LastName"].ToString() + ", " + reader["FirstName"].ToString();
                        documentdata.Append(" ItemURLText=\"");
                        documentdata.Append(name.Replace("\"", "'"));
                        documentdata.Append("\"");
                    }
        ...
        public void DrawProfilesModuleOld()
        {
            // UCSF, Search for GetSimilarPeopleSameInstitution in DataIO and use that
