...
/****** Object:  StoredProcedure [Profile.Data].[Publication.Entity.UpdateEntityOnePerson]    Script Date: 11/17/2017 12:39:41 PM ******/
SET ANSI_NULLS ON
GO
	...
	INSERT  INTO #Publications
            ...
            SELECT -- Get Pub Med pubs
                    ...
                    Reference = REPLACE([UCSF.CTSASearch].[fnPublication.Pubmed.General2Reference](PG.PMID,
                                        ...
                                        CHAR(11), '') ,
                    Source = CASE 
						when PG.pmid > 0 then 'PubMed'
						else WEBPAGE.DBType
					end,
					URL = Case 
						when PG.pmid > 0 then 'http://www.ncbi.nlm.nih.gov/pubmed/' + CAST(ISNULL(PG.pmid, '') AS VARCHAR(20))
						else WEBPAGE.url
					end,
			...
			join [UCSF.].[Publication.URL] WEBPAGE on PG.pmid=WEBPAGE.pmid
			WHERE	PG.PMID IN (
						SELECT PMID 
