...
drop table EmployeeClassesInImport
SELECT 
/*
...
select IsNULL(sc.EmployeeClassDescription,'Others') EmployeeClassDescription,sc.all_cnt 'In Current Import',
	isNULL(scprof.cnt,0) 'In Profiles',isNull(scvip.cnt,0) 'From VIP'
	from EmployeeClassesInImport sc
...
left outer	join EmployeeClassesInProfilesFromVIP scvip 
		on sc.EmployeeClassDescription=scvip.EmployeeClassDescription
--where sc.EmployeeClassDescription is not NULL
UNION
select 'Summary',sum(sc.all_cnt),sum(scprof.cnt),sum(scvip.cnt)
from EmployeeClassesInImport sc
left outer	join EmployeeClassesInProfiles scprof 
		on sc.EmployeeClassDescription=scprof.EmployeeClassDescription
left outer	join EmployeeClassesInProfilesFromVIP scvip 
