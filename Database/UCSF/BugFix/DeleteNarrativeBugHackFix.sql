/****** Object:  StoredProcedure [RDF.].[GetStoreTriple]    Script Date: 6/13/2019 11:54:46 AM ******/
SET ANSI_NULLS ON
GO
	...
	BEGIN
		...
		SELECT @OldTripleID = TripleID,
			...
			WHERE subject = @SubjectID AND predicate = @PredicateID AND (object = @OldObjectID 
				OR 1 = (SELECT max(MaxCardinality) FROM [Ontology.].[ClassProperty] WHERE MaxCardinality IS NOT NULL AND
